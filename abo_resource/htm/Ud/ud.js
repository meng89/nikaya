...
function get_str_title()
{
	...
	var sutranum = myurl.replace(/^.*(\d\d)\.htm/i,"$1");
// 取出經號
	var next_sutra = parseInt(sutranum,10) + 1; 
// 下一經
if(next_sutra > 80) {next_sutra = 1;}
	next_sutra = "00" + next_sutra.toString();
	next_sutra = next_sutra.replace(/^.*(\d\d)/,"$1");
	var pre_sutra = parseInt(sutranum,10) - 1; 
// 前一經
	if(pre_sutra < 1){pre_sutra = 80;}
	pre_sutra = "00" + pre_sutra.toString();
	pre_sutra = pre_sutra.replace(/^.*(\d\d)/,"$1");
	var str_title = '<center><font color="#3366FF"><span style="font-size:14pt; font-family:新細明體"><a href="../index.htm">首 頁</a>　 <a href="index.htm">目錄/凡例</a>　 <a href="Ud' + pre_sutra + '.htm">上一經</a>/<a href="Ud' + next_sutra + '.htm">下一經</a>　　　　　　<a href="https://nikaya.buddhason.org/Ud/Ud' + sutranum + '.htm">鄭 健筆記</a></font></center><div id="main_menu"></div>';
	return str_title;
}
 ...
 },
{
                   region: 'center',
                    contentEl: 'center',
                    title: '<font color="#3366FF"><span style="font-size:14pt">漢譯優陀那(自說經)經文</font>',
                    autoScroll: true,
                    collapsible: true,
                    width: '50%',
                    border: false,
