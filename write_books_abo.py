...
def main(nopdf, noepub):
    ...
    for count, m in enumerate(all_modules, start=1):
        ...
        for zh_name, lang in [("莊春江汉译经藏_简体PDF", abo.ebook_utils.SC()), ("莊春江漢譯經藏_繁體PDF", abo.ebook_utils.TC())]:
            if nopdf:
                continue
            for size in ("A4",):
                ...
                if m in abo.kn.all_modules:
                    full_path = os.path.join(dirname, "小部(不全)", filename)
                else:
                    full_path = os.path.join(dirname, filename)
                os.makedirs(os.path.dirname(full_path), exist_ok=True)
                job = (filename, abo.pdf.build_pdf, (full_path, data, m, lang, size, True))
                jobs.append(job)
                total += 1
                try_run_job()

        for zh_name, lang in [("莊春江汉译经藏_简体EPUB", abo.ebook_utils.SC()), ("莊春江漢譯經藏_繁體EPUB", abo.ebook_utils.TC())]:
            if noepub:
                continue
            ...
            jobs.append((filename, abo.epub.build_epub, (full_path, data, m, lang, True)))
            total += 1
            try_run_job()
