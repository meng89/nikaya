...
import pyabo
import pyabo2.kn
from pyabo import parse_original_line
...
def htm2xml(module, filename):
    full_path = os.path.join(config.DOWNLOAD_DIR, filename)
    htm_data = open(full_path, "r").read()

    mtime = os.path.getmtime(full_path)

    soup = bs4.BeautifulSoup(htm_data, 'html5lib')
    x = soup.prettify()
...
def read_page(filename, local_notes):
    full_path = os.path.join(config.DOWNLOAD_DIR, filename)
    htm_data = open(full_path, "r").read()
    ...
    pali_text = soup.find("div", {"class": "pali"}).text
    inside_notes = _do_class_comp(soup.find("div", {"class": "comp"}),
    ...
    homage_and_head_lines, sutta_name_part, translator_part, agama_part, body_lines = \
        _do_class_nikaya(list(soup.find("div", {"class": "nikaya"}).contents),
                         url_path=url_path, sutta_temp_notes=inside_notes, local_notes=local_notes)
    ...
    return (homage_line, head_lines, sutta_name_part, translator_part, agama_part,
            body_lines, pali_text, last_modified)
