...
def load_from_htm():
    ...
    for htm in htmls:
        ...
        jis = abo.utils.split_sutta(nikaya_lines, matchs)

        _, _head_lines , _ = jis[0]

        pian_p = re.compile(r"^(\d+)\.(.+集篇)$")
        pian_matchs = abo.utils.match_line(_head_lines, [pian_p])
        if pian_matchs:
        ...
        pin_matchs = abo.utils.match_line(_head_lines, [pin_p])
        if pin_matchs:
        ...
        for index, (source_title_line, head_lines, body_lines) in enumerate(jis):
            ...
            _serial, title_line = abo.utils.split_serial_title(source_title_line)
